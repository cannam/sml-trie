...
functor TrieMapKeyAdapterFn (A : TRIE_MAP_KEYADAPTER_FN_ARG)
        :> TRIE_MAP
               where type key = A.external_key = struct
...
functor PatternMatchTrieMapKeyAdapterFn (A : PATTERN_MATCH_TRIE_MAP_KEYADAPTER_FN_ARG)
        :> PATTERN_MATCH_TRIE_MAP
               where type element = A.T.element where type key = A.external_key = struct

    structure T = A.T

(*!!! simplify by using TrieMapKeyAdapterFn above?
 also, what about this naming eh? *)
